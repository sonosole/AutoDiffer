...
l = rand(10, 62)
l = l ./ sum(l,dims=1)
m = MLP([256, 128,32,16,8, 10], [sin,relu,softplus,sigmoid,softmax])

# [2] forward and backward propagation
o = forward(m, Variable(x))
loss = crossEntropy(o, Variable(l))
LOSS1 = cost(loss)
backward()
GRAD = m.parameter[1].w.delta[1,1]
println("\n---------------------------------")
println("反向传播梯度: ", GRAD)
...
DELTA = 1e-7
m.parameter[1].w.value[1,1] += DELTA
