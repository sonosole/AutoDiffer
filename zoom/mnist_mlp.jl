...
operator = [relu, relu, relu, relu, softmax]
...
function train(Xtrain, Ytrain, Xtest, Ytest, mlpmodel)
    ...
    for e = 1:epoch
        ...
        for b = 1:batchnums
            ...
            prob = forward(mlpmodel, Variable(input))
            cost = crossEntropyLoss(prob, Variable(label))
            backward()
            if b%8==0
                update(optimizer,parameter;clipvalue=50)
                zerograds(parameter)
            end
            lossvalue[b] = cost.value / batchsize
        end
...
function test(Xtest, Ytest, mlpmodel)
    ...
    for i = 1:num
        ...
        result = argmax(predict(mlpmodel, feats))[1]
